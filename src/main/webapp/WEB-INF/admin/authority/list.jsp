	...
	<div class="page-control" data-role="page-control">
		...
		<ul>
			<li class="active"><a href="#page1"><i
					class="icon-equalizer"></i>权限列表</a></li>
			<li><a href="#page2">添加权限</a></li>
		...
		<div class="frames">
			...
			<thead>
				<tr>
					<th style="text-align:left; width:40% ">权限名称</th>
					<th style="text-align:left; width:30%">权限描述</th>
			...
			<tbody>
				<c:forEach items="${page.getContent() }" var="authority" >
					<tr>
						...
						<td>
							<c:if test = "${authority.status =='enabled'}">已启用</c:if>
							<c:if test = "${authority.status =='disabled'}">已禁用</c:if>
						</td>
						<td>
							<c:if test = "${authority.level =='ordinary'}">普通的</c:if>
							<c:if test = "${authority.level =='systemic'}">系统的</c:if>
						</td>
                ...
                <div id="pageDiv"></div>
                <%
                	Page<Authority> _page = (Page<Authority>)request.getAttribute("page");
